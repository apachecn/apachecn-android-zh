...
通常在我们的应用程序中，我们会希望向用户显示一些信息，或者要求在弹出窗口中确认某个操作。这就是所谓的**对话框**窗口。如果你快速扫描AndroidStudio的调色板，你可能会惊讶地发现没有提到任何对话窗口。
...
在AndroidStudio中创建一个新类，方法是右键单击包含你的包名的文件夹(包含`MainActivity.kt`文件的文件夹)。选择**新建** | **柯特林文件/类**，命名为`MyDialog`，在下拉选择器中选择**类**。左键点击**确定**创建类。
