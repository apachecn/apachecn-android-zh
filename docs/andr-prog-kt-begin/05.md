...
检查`MainActivity.kt`文件，你会看到它是我们在第一个项目中使用的柯特林文件的简化版本。都是一样的，只是`onCreate`功能的功能少，代码少。功能缺失是因为 UI 比较简单，不需要；因此，AndroidStudio没有生成它们。
...
大多数现代应用程序都是为尽可能多的受众制作的。此外，如果应用程序有很大的规模或复杂性，那么软件公司中的角色通常被分成许多不同的团队。例如，为安卓应用程序编写Kotlin代码的人很可能与设计用户界面的布局没有什么关系。
...
在这个阶段，我们已经很好地掌握了安卓项目中的内容以及它们是如何链接在一起的。现在让我们再构建一个应用程序，看看不同的应用程序模板对AndroidStudio生成的底层文件有何不同。
...
现在，您可以运行模拟器上安装的任何应用程序。请注意，当您通过AndroidStudio运行您的应用程序之一时，它仍然安装在模拟器上，因此可以从应用程序抽屉中运行。但是，您在AndroidStudio中对应用程序所做的每一个更改都将要求您通过单击AndroidStudio快速启动栏上的播放按钮来再次运行或安装应用程序，就像我们一直在做的那样。
