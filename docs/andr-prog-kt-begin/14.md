...
*   如果我们试图引用一个不存在的对象，我们将得到一个**空指针异常**错误，应用将崩溃。Kotlin的主要特点之一是它保护我们免受这种情况的影响。在Kotlin试图改进的 Java 中，空指针异常错误是应用崩溃最常见的原因。我们将在本章末尾名为*可空性–*`val`*和* `var` *的章节中了解更多关于 Kotlin 如何帮助我们避免 **NullPointerException** 错误的信息。*
...
请注意在这个应用中不需要`Log`或`Toast`，因为我们最终使用Kotlin代码来操作用户界面。
