...
我们可以调用函数的各种方法非常有用。在其他编程语言中，当您希望能够以不同的方式调用相同的命名函数时，您必须提供该函数的多个版本。虽然学习命名参数和默认参数可能会增加一点复杂性，但这肯定比必须编写四个版本的`orderProduct`函数要好。这一点，以及类型推断，只是你经常会听到程序员赞美Kotlin简洁本质的两个原因。
