...
在这个阶段，我们已经很好地掌握了安卓项目的内容以及不同方面是如何联系在一起的。让我们再构建一个应用，不是详细介绍它，而是看看不同的应用模板对AndroidStudio生成的底层文件的不同之处。
...
现在，您可以运行模拟器上安装的任何应用。请注意，当您通过AndroidStudio运行您的应用之一时，它仍然安装在模拟器上，因此可以从应用抽屉中运行。然而，您在AndroidStudio中对应用所做的每一个更改都需要您通过单击AndroidStudio快速启动栏上的播放按钮来再次运行/安装应用——就像我们一直在做的那样。
