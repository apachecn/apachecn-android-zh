...
`TextView`小部件的位置很整齐，就好像`ImageView`小部件在那里一样，但是从前面的截图中我们可以看到，AndroidStudio中可见的头像图标是不存在的。这样做的原因是因为我们从**样本数据**类别中选择了图像。AndroidStudio允许我们使用样本数据，这样我们就可以在图像可用之前开始布局我们的应用。这很有用，因为在应用的整个开发生命周期中，需要生成/获取图像是很常见的。
