...
我们制作了第一个屏幕，并为应用本身定义了一些细节。为了确保我们到目前为止所做的是正确的，我们构建并运行我们的应用。我们将运行完整的调试构建变体。如果你忘记了如何切换到这个版本，我们会提醒你。打开AndroidStudio和`Journaler`项目。单击AndroidStudio窗口左侧的构建变体窗格，或者选择查看|工具窗口|构建变体，打开构建变体窗格。将出现“构建变体”窗格。从下拉列表中选择“完成调试”，如屏幕截图所示:
...
您可以通过这些日志级别(例如，当您只需要查看错误-应用崩溃 stacktrace)或日志标记(我们将在后面解释)来过滤日志消息；或者通过关键字、正则表达式或应用包。在我们应用任何过滤器之前，我们将配置AndroidStudio，以便日志消息有不同的颜色。
