...
字体是资产。所以为了给你的应用程序提供一些字体，我们需要先复制它们。有很多好的免费字体资源。比如谷歌字体。下载一些字体，复制到你的`assets`目录。如果没有`assets`目录，创建一个。我们将把我们的字体放在`assets/fonts`目录中。
...
现在所有应用程序的按钮都将应用粗体版本。现在构建您的应用程序并运行它。你会注意到字体变了！
