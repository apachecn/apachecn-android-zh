...
如果你是继续下一部分，那么离开这个屏幕。否则，您可以关闭它，并从窗口开始菜单运行AndroidStudio，就像任何其他应用一样，当您准备好继续时。
...
当我们生成该项目时，AndroidStudio构建了一个更适合常规应用和常规用户界面的项目。我们已经通过修改 XML 隐藏了操作栏。现在，我们将在`SubHunter.java`文件中进行一些小的代码更改，以使用更适合游戏开发的安卓类。
