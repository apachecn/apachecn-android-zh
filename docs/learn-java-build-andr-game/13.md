...
在AndroidStudio中，在项目浏览器窗口中找到`app/res/drawable`文件夹。下面的截图清楚地显示了该文件夹的位置:
...
AndroidStudio剖析工具相当复杂和深入。然而，用我们的游戏做一些真正有意义的测量是非常简单的。我们可以看到我们的应用程序使用了多少设备资源，因此试图提高我们游戏的效率，使其运行更高效，使用更少的资源。通过资源，我说的是 CPU 和内存的使用。

代码优化超出了本书的范围，但是看看我们如何开始监控我们游戏的性能是一个很好的介绍。从AndroidStudio主菜单中选择**查看**，然后选择**工具窗口** | **侧写**。
