...
AndroidStudio有很多错误，但只是因为代码引用了我们还没有编码的`BulletHellGame`类。我们现在就去做。
...
请记住，当我们将所有输出发送到AndroidStudio的 **logcat** 窗口时，模拟器显示屏上不会发生任何事情(但是您需要一个模拟器来运行应用程序)。以下是输出:
...
运行示例应用程序，记住当我们将输出发送到AndroidStudio的 **logcat** 窗口时，模拟器屏幕上不会发生任何事情。以下是输出:
...
运行这个例子，记住屏幕上不会发生任何事情，因为所有的输出都将被发送到AndroidStudio的 **logcat** 控制台窗口。以下是输出:
