...
我们可以使用可选的`tools:listitem`属性来告诉AndroidStudio在我们的预览工具栏中放大哪个布局作为列表项。这将让我们了解`RecyclerView`在我们的应用中的外观。
...
具体来说，在`CatViewHolder`中，由于一些Kotlin魔法，我们可以使用`define val catData = listItem.data`而保持类的其余部分不变。
