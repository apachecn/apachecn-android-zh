# 十六、在Google Play上启动你的应用

概观

本章将向您介绍 Google Play 控制台、发布渠道以及整个发布过程。它包括创建一个Google Play开发者帐户，为我们开发的应用设置商店条目，以及创建一个密钥商店(包括密码的重要性和存储文件的位置)。我们还将了解应用捆绑包和 APK，了解如何生成应用的 APK 或 AAB 文件。在本章的后面，我们将设置发布路径，打开 beta，关闭 alpha，最后我们将把我们的应用上传到商店，并在设备上下载。

在本章结束时，您将能够创建自己的Google Play开发者帐户，为发布准备您的签名 APK 或应用包，并在Google Play上发布您的第一个应用。

# 简介

在前一章中，您已经学习了如何使用`CoordinatorLayout`和`MotionLayout`添加动画和过渡。现在，您已经准备好开发和启动安卓应用了。

开发安卓应用后，它们将只在你的设备和模拟器上可用。你必须让每个人都可以使用它们，这样他们就可以下载它们。反过来，你将获得用户，你可以从中获利。安卓应用的官方市场是Google Play。有了Google Play，你发布的应用和游戏可以在全球超过 20 亿活跃的安卓设备上使用。

在这一章中，我们将学习如何在Google Play上启动你的应用。我们将从准备应用发布开始，并创建一个Google Play开发者帐户。然后，我们将继续上传您的应用并管理应用发布。

让我们开始准备您的应用，以便在Google Play上发布。
...
安卓应用运行时，会自动通过调试密钥进行签名。然而，在发布到谷歌 Play 商店之前，应用必须使用发布密钥进行签名。为此，您必须有一个密钥库。如果你还没有，你可以在AndroidStudio创建一个。

## 练习 16.01:在 Android Studio 中创建密钥库

在本练习中，我们将使用AndroidStudio制作一个密钥库，用于签署安卓应用。按照以下步骤完成本练习:
...
您的密钥库将用于签署您的应用以供发布，以便您可以在Google Play上发布它。我们将在下一节讨论这个问题。

## 签署让你的应用发布

当您在模拟器或实际设备上运行应用时，AndroidStudio会自动使用调试密钥库对其进行签名。要在 Google Play 上发布，您必须使用自己在 Android Studio 中制作的密钥库或从命令行使用自己的密钥签署 APK 或应用捆绑包。
...
在本练习中，我们将使用AndroidStudio为安卓项目创建一个签名的 APK:
...
4.  选择您在*练习 16.01* 、*中制作的密钥库，在AndroidStudio*中创建密钥库。
...
12.  点击`Finish`按钮。AndroidStudio将建造签署的 APK。将弹出一个 IDE 通知，通知您已生成签名的 APK。您可以点击`locate`进入签署的 APK 文件所在的目录:
...
安卓应用捆绑包是一种新的发布应用打包方式。您只需生成一个应用捆绑文件(使用AndroidStudio 3.2 及更高版本)，并将其上传到谷歌 Play 上。Google Play 将自动为每个设备配置、CPU 架构和语言生成基本的 APK 文件和 APK 文件。当用户安装你的应用时，他们只会为他们的设备下载必要的 apk。与通用 APK 相比，它的体积会更小。

这将适用于安卓 5.0 棒棒糖及以上的设备；对于下面的文件，将生成的 APK 文件仅用于设备配置和 CPU 架构。所有语言和其他资源都将包含在每个 APK 文件中。

## 练习 e 16.03:创建已签名的应用捆绑包

在本练习中，我们将使用AndroidStudio为安卓项目创建一个签名的应用捆绑包:
...
11.  点击`Finish`按钮。AndroidStudio将构建签名的应用捆绑包。将弹出一个集成开发环境通知，通知您已生成签名的应用捆绑包。您可以点击`locate`进入已签名的 app bundle 文件所在的目录:
...
为了能够以安卓应用捆绑格式发布您的应用，您需要选择通过Google Play进行应用签名。我们将在下一节讨论Google Play应用签名。
...
发布新应用时，很容易选择应用签名。在Google Play控制台([https://play.google.com/console](https://play.google.com/console))中，你可以进入`Release Management` | `App Releases`部分，在`Let Google manage and protect your app signing key`部分选择`Continue`。您最初用于签署应用的密钥将成为上传密钥，谷歌 Play 将生成新的应用签署密钥。

您也可以将现有应用转换为使用应用签名。这可以在Google Play控制台的应用的`Release` | `Setup` | `App Signing`部分找到。您需要上传现有的应用签名密钥并生成新的上传密钥。

一旦您注册了Google Play应用签名，您将不再能够选择退出。此外，如果您使用第三方服务，您将需要使用应用签名密钥的证书。这在`Release Management` | `App Signing`中有。

应用签名还可以让你上传应用包，谷歌 Play 会自动签名并生成 APK 文件，用户在安装你的应用时会下载这些文件。

在下一部分，您将创建一个Google Play开发者帐户，这样您就可以向Google Play发布一个应用的 APK 签名或应用捆绑包。
...
您需要支付 25 美元的注册费来创建您的Google Play开发者帐户。这是一次性付款)。费用必须使用有效的借记卡/信用卡支付，但是一些预付费/虚拟信用卡也可以。您可以使用的内容因地点/国家而异。
...
当您收到来自谷歌的确认电子邮件时，您可以开始向Google Play发布应用和游戏。

在下一部分，我们将讨论上传应用到Google Play。

# 上传一个 n 应用到Google Play

一旦你有了可以发布的应用和Google Play开发者账号，你就可以去Google Play控制台([https://play.google.com/console](https://play.google.com/console))发布应用了。
...
商店列表是用户在Google Play上打开你的应用页面时首先看到的。如果 app 已经发布，可以先进入`Grow` 再进入`Store presence`，然后选择`Main store listing`。
...
确保应用捆绑包或 APK 文件由您的发行版签名密钥签名。如果没有正确签名，Google Play控制台将不会接受它。如果要发布更新，应用捆绑包或 APK 的版本代码必须高于现有版本。
...
如果对发布有信心，可以选择页面底部的`START ROLLOUT TO PRODUCTION` 按钮。发布应用后，需要一段时间(新应用需要 7 天或更长时间)才能进行审查。你可以在Google Play控制台的右上角看到状态。这些状态包括以下内容:

*   待发布(您的新应用正在审核中)
*   已发布(您的应用现在可以在谷歌 Play 上使用)
*   已拒绝(由于违反策略，您的应用未发布)
*   已暂停(您的应用违反了Google Play政策，已被暂停)

如果您的应用有问题，您可以解决这些问题并重新提交应用。您的应用可能会因版权侵权、假冒和垃圾邮件等原因而被拒绝。

一旦应用发布，用户现在就可以下载了。新应用或应用更新在谷歌 Play 上上线可能需要一段时间。如果你试图在Google Play上搜索你的应用，它可能是不可搜索的。请确保您在制作或开放轨道上发布它。
...
要前往每个曲目并管理版本，您可以前往Google Play控制台的`Release` 部分，选择`Production`或`Testing`，然后选择开放、关闭或内部曲目。
...
在创建和发布要控制发布的更新之前，必须设置托管发布。当您在Google Play控制台上选择您的应用时，您可以选择左侧的`Publishing Overview`。在`Managed publishing status`部分，点击`Manage`按钮:
...
作为这本书的最后一项活动，你的任务是创建一个Google Play开发者账户，并发布一个你新开发的安卓应用。您可以发布作为本书一部分而构建的某个应用，或者您正在开发的另一个项目。您可以使用以下步骤作为指南:

1.  转到Google Play开发者控制台([https://play.google.com/console](https://play.google.com/console))并创建一个帐户。
2.  创建一个密钥库，用于签署发布版本。
3.  生成一个安卓应用包进行发布。
...
这一章涵盖了谷歌 Play 商店:从准备发布，到创建Google Play开发者帐户，到最终发布你的应用。我们首先对您的应用进行版本控制，生成密钥库，创建 APK 文件或安卓应用捆绑包，并使用发布密钥库对其进行签名，然后存储密钥库及其凭据。然后，我们继续在Google Play控制台上注册一个帐户，上传你的 APK 文件或应用包，并管理发布。

这是本书所做工作的高潮——发布你的应用并向世界开放是一项伟大的成就，展示了你在整个课程中取得的进步。

在这本书里，你学到了很多技能，从安卓应用开发的基础知识开始，一直到实现诸如`RecyclerViews`、从网络服务获取数据、通知和测试等功能。你已经看到了如何用最佳实践、架构模式和动画来改进你的应用，最后，你已经学会了如何在Google Play上发布它们。
