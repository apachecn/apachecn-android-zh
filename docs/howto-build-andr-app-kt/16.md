...
2.  Go to the `Build` menu and then click on `Generate Signed Bundle or APK...`:
    ...
    ![Figure 16.1: The Generate Signed Bundle or APK dialog ](image/B15216_16_01.jpg)
...
3.  Make sure either `APK` or `Android App Bundle` is selected, and then click the `Next` button. Here, you can choose an existing keystore or create a new one:
    ...
    ![Figure 16.2: The Generate Signed Bundle or APK dialog after selecting APK  and pressing the Next button ](image/B15216_16_02.jpg)

    图 16.2:选择 APK 并按下“下一步”按钮后，生成签名包或 APK 对话框

4.  Click the `Create new…` button. The `New Key Store` dialog will then appear:

    ![Figure 16.3: The New Key Store dialog ](image/B15216_16_03.jpg)
...
2.  Go to the `Build` menu and then click on the `Generate Signed Bundle or APK…` menu item:
    ...
    ![Figure 16.4: The Generate Signed Bundle or APK Dialog ](image/B15216_16_04.jpg)

    图 16.4:生成签名包或 APK 对话框

3.  Select `APK` and then click the `Next` button:

    ![Figure 16.5: The Generate Signed Bundle or APK dialog after clicking the Next button ](image/B15216_16_05.jpg)
...
10.  In the `Build Variants` field, make sure the `release` variant is selected:
    ...
    ![Figure 16.6: Choose the release build in the Generate Signed Bundle or APK dialog ](image/B15216_16_06.jpg)
...
![Figure 16.7: Pop-up notification for successful signed APK generation ](image/B15216_16_07.jpg)
...
2.  Go to the `Build` menu, then click on the `Generate Signed Bundle or APK…` menu item:
    ...
    ![Figure 16.8: The Generate Signed Bundle or APK dialog ](image/B15216_16_08.jpg)

    图 16.8:生成签名包或 APK 对话框

3.  Select `Android App Bundle`, then click the `Next` button:

    ![Figure 16.9: The Generate Signed Bundle or APK dialog after clicking the Next button ](image/B15216_16_09.jpg)
...
10.  In the `Build Variants` field, make sure the `release` variant is selected:
    ...
    ![Figure 16.10: Choose the release build in the Generate Signed Bundle or APK dialog ](image/B15216_16_10.jpg)
...
![Figure 16.11: Pop up notification that the signed app bundle was generated ](image/B15216_16_11.jpg)
...
![Figure 16.12: A draft release on the production track ](image/B15216_16_12.jpg)
...
![Figure 16.13: The dialog for halting a staged rollout ](image/B15216_16_13.jpg)

图 16.13:暂停分阶段部署的对话框

当分阶段的卷展栏暂停时，您的轨迹页面中的发布页面将更新为`Rollout halted` 文本和一个`Resume rollout`按钮:

![Figure 16.14: The release page for a halted staged rollout ](image/B15216_16_14.jpg)
...
![Figure 16.15: Managed publishing on Publishing overview ](image/B15216_16_15.jpg)
...
![Figure 16.16: Managed publishing Changes ready to publish ](image/B15216_16_16.jpg)
