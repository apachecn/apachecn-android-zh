...
如果您正在使用RetroFit，您可以通过添加一个调用适配器工厂将响应转换为 RxJava 可观察到的。首先，您需要在您的`app/build.gradle`文件依赖项中添加`adapter-rxjava3`，如下所示:
...
现在，您的RetroFit方法可以返回您可以在代码中收听的`Observable`对象。例如，在您调用电影端点的`getMovies`RetroFit方法中，您可以使用以下方法:
...
如果您使用的是RetroFit 2.6.0 或更高版本，您可以使用`suspend`将端点功能标记为暂停功能:
...
1.  在AndroidStudio打开`Popular Movies`项目。
...
12.  After that, add an observer to the `getPopularMovies` and `error` LiveData from `movieViewModel`:
    ...
    这将使用获取的影片更新活动的 RecyclerView。电影列表使用Kotlin的`filter`功能过滤，只包括今年上映的电影。然后使用Kotlin的`sortedBy`功能按标题排序。
...
1.  在AndroidStudio中，打开你在上一个练习中处理过的`Popular Movies`项目。
...
1.  在AndroidStudio新建一个项目，并命名为`TV Guide`。设置它的包名。
...
14.  创建一个`TelevisionService`类来添加RetroFit方法。
