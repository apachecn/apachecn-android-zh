...
3.  In the `Configure Your Project` dialog, as shown in *Figure 15.1*, name the project `Tip Calculator` and set the package name as `com.example.tipcalculator`:
    ...
    ![Figure 15.1: Configure Your Project dialog ](image/B15216_15_01.jpg)
...
![Figure 15.2: The app screens: input screen (on the left) and output screen (on the right) ](image/B15216_15_02.jpg)
...
![Figure 15.3: The app screens: input screen (on the left) and output screen (on the right) ](image/B15216_15_03.jpg)
...
![Figure 15.4: The left screenshot displays the UI before and after the Snackbar is shown. The screenshot on the right shows the UI while the Snackbar is visible ](image/B15216_15_04.jpg)
...
![Figure 15.5: The starting animation (left) and ending animation (right) ](image/B15216_15_05.jpg)
...
![Figure 15.6: The Motion Editor in Android Studio 4.0 ](image/B15216_15_06.jpg)
...
![Figure 15.7: The Motion Editor's Overview panel with start_constraint selected ](image/B15216_15_07.jpg)
...
![Figure 15.8: The Motion Editor’s Selection panel showing ConstraintSet for start_constraint ](image/B15216_15_08.jpg)

图 15.8:运动编辑器的选择面板显示 start_constraint 的约束集

点击`Overview`面板左侧的`MotionLayout`，下方的`Selection`面板将显示视图及其约束，如图*图 15.9* :

![Figure 15.9: The Overview and Selection panel when MotionLayout is selected ](image/B15216_15_09.jpg)
...
![Figure 15.10: How the Motion Editor looks when start_constraint is selected ](image/B15216_15_10.jpg)

图 15.10:选择 start_constraint 时运动编辑器的外观

*图 15.11* 显示了如果选择`end_constraint`运动编辑器的外观:

![Figure 15.11: How the Motion Editor looks with end_constraint selected ](image/B15216_15_11.jpg)
...
![Figure 15.12: The transition in the middle of the animation ](image/B15216_15_12.jpg)
...
![ Figure 15.13: Using SHOW_PATH (left) shows the animation path, while SHOW_PROGRESS (right) shows the animation progress ](image/B15216_15_13.jpg)
...
![Figure 15.14: The OK button is blocking the middle of the text animation ](image/B15216_15_14.jpg)
...
5.  Run the app and make a computation. On the output screen, tap on the image. Look at the tip text while the animation is in progress. You will notice that the text is covered by the image in the middle of the animation, as shown in *Figure 15.15*:
    ...
    ![Figure 15.15: The image hides the TextView displaying the tip ](image/B15216_15_15.jpg)
...
7.  In the `Overview` panel at the top right, click the arrow connecting `start_constraint` and `end_constraint`. Drag the down arrow in the `Selection` panel to the middle (50%), as shown in *Figure 15.16*:
    ...
    ![Figure 15.16: Select the arrow representing the transition between  the start and end constraints ](image/B15216_15_16.jpg)
...
8.  Click the `Create KeyFrames` icon to the right of `Transition` in the `Selection` panel (the one with a green `+` symbol). Refer to *Figure 15.17* to see the icon:
    ...
    ![Figure 15.17: The Create KeyFrames icon ](image/B15216_15_17.jpg)
...
10.  Select `ID`, choose `image`, and set the input position to `50`. The `Type` is `parentRelative`, and `PercentX` is `1.5`, as *Figure 15.18* shows. This will add a `KeyPosition` attribute for the image at the middle (50%) of the transition, which is at 1.5 times relative to the `x` axis of the parent:
    ...
    ![Figure 15.18: Provide the input to the key position to be made ](image/B15216_15_18.jpg)
...
11.  Click the `Add` button. You will see in the `Design` preview, as shown in *Figure 15.19*, that the motion path is no longer a straight line. At position 50 (the middle of the animation), the text will no longer be covered by the `ImageView`. The `ImageView` will be to the right of the `TextView`:
    ...
    ![Figure 15.19: The path will now be curved instead of straight. The Transition  panel will also have a new item for KeyPosition ](image/B15216_15_19.jpg)
...
12.  Click the play icon to see how it will animate. Run the application to verify it on a device or emulator. You will see that the animation is now curving to the right instead of taking its previous straight path, as in *Figure 15.20*:
    ...
    ![Figure 15.20: The animation now avoids the TextView with the tip ](image/B15216_15_20.jpg)
...
![Figure 15.21: The start and end state of MotionLayout in the Password Generator app ](image/B15216_15_21.jpg)
