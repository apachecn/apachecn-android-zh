...
使用 Java 和许多其他语言的主要难点之一与访问空引用有关。这可能会导致应用程序崩溃，而不会向用户显示足够的错误消息。如果你是一名 Java 开发人员，我很确定你对全能的`NullPointerException`非常熟悉。Kotlin最神奇的一点就是零安全。
...
*   使用特定 SDK 帮助开发人员做出明智选择的设备百分比数据。要在AndroidStudio中查看这些数据，请单击最小软件开发工具包下拉菜单下的帮助我选择。这将向您显示当前支持的 Android SDK 版本及其支持功能的列表，以及如果您选择该版本作为最低 SDK，您的应用程序将支持的 Android 设备的百分比:
...
从模拟器运行应用程序非常容易。点击AndroidStudio工具栏上的播放图标，如下图截图所示:
...
AndroidStudio将在模拟器上构建和运行您的应用程序:
...
在这一章中，我们经历了检查和安装 JDK 的过程，这是安卓开发所必需的。我们还安装和设置了我们的AndroidStudio环境。我们创建了第一个安卓应用程序，并学会了在模拟器和实际设备上运行它。
