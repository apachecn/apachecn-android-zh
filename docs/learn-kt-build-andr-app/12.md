...
你首先要把 **SDK** ( **软件开发工具包**)添加到你的应用中。你应该确保你的目标至少是安卓 4.0(冰淇淋三明治)。它应该安装谷歌 Play 商店应用程序，或者运行安卓 4.0 和谷歌应用程序接口的模拟器。你的AndroidStudio版本应该至少是 2.2。您将使用AndroidStudio中的 Firebase 助手窗口来进行集成。
