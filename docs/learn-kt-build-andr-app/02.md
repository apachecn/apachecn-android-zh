...
到目前为止，我们已经完成了创建一个 Kotlin 类并在我们的`MainActivity.java`类中访问它的方法的过程。我们的项目目前由一个 Java 类和一个 Kotlin 类组成，但是我们希望我们的整个项目都在 Kotlin 中。那么，我们该怎么办？我们一定要重写Kotlin的`MainActivity.java`课吗？不。Kotlin插件给安卓工作室增加的功能之一是将代码从 Java 转换成Kotlin的能力。
