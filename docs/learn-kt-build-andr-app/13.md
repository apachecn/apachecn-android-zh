...
要开始构建我们的笔记应用程序，请创建一个新的应用程序，并将其称为笔记应用程序。使用AndroidStudio左上角的选项卡切换到项目视图。此视图允许您查看文件系统中存在的完整项目结构。它应该如下图所示:
...
您将使用AndroidStudio 2.2+中的 Firebase 助手工具将您的应用程序连接到 Firebase。助手工具将更新您现有的项目，或者创建一个包含所有必要的渐变依赖项的新项目。它提供了一个非常直观的用户界面指南，您可以遵循:
...
之后，您应该会看到与AndroidStudio集成的 Fabric 插件。使用您的电子邮件和密码注册:
