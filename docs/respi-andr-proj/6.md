...
我们可以像以前一样在AndroidStudio创建一个新的应用。我们在这个应用中不需要任何`Activity`。
...
我们将构建自己的应用来下载测量值并在地图上显示。在AndroidStudio创建一个新的空白项目，并选择在创建项目向导的最后一步包括一个谷歌地图活动。我已经用安卓 4.3 作为这个项目的基础 SDK 我将我的主要活动命名为`MapsActivity`。
