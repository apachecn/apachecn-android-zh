...
下一步是在安卓系统上安装更好的 VLC 客户端。我们将使用 **VLC 进行安卓测试版**应用。安装后，再次打开 RaspiCam Remote 应用程序，然后点击摄像头图标开始流媒体。此时，安卓会要求你为安卓测试版选择标准视频播放器或新安装的 VLC。选择后者，你会体验到更好的流媒体质量。不要忘记将端口`8080`添加到路由器的端口转发设置中，以便通过互联网访问流式视频。
