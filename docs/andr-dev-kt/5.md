...
Kotlin 支持函数式编程，函数在 Kotlin 是一等公民。在给定的编程语言中，一级公民是一个术语，它描述了一个实体，该实体支持其他实体通常可以使用的所有操作。这些操作通常包括作为参数传递、从函数返回以及分配给变量。句子“*一个函数在Kotlin*中是一等公民”那么应该理解为:*在Kotlin中可以传递函数作为参数，从函数中返回它们，并将其分配给变量*。虽然 Kotlin 是一种静态类型的语言，但是需要定义一个函数类型来允许这些操作。在Kotlin中，用于定义函数类型的符号如下:
...
带有函数参数的函数可能与本机结构(如循环)类似。我们已经看到了`ifSupportsLolipop`功能和`repeatUntilError`功能。一个更常见的例子是`forEach`修饰语。它是`for`控制结构的替代方案，它用一个接一个的元素调用一个参数函数。这就是它的实现方式(在Kotlin标准库中有一个`forEach`修改器，但是我们稍后会看到它，因为它包含了尚未呈现的元素):
