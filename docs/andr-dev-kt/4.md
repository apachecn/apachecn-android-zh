...
Kotlin中的类是使用`class`关键字*定义的。*下面是最简单的类声明——一个名为`Person`的空类:
...
我们已经知道，所有Kotlin类型的超类型是`Any`。它相当于 Java `Object`类型。每个Kotlin类都显式或隐式地扩展了`Any`类。如果我们不指定父类，`Any`将被隐式设置为该类的父类:
...
这种情况下引入了大量样板代码，这些代码只用于将调用委托给其他构造函数的构造函数。Kotlin对这个问题的解决方案是使用`@JvmOverload`注释:
...
在[第二章](2.html#11C3M0-7a7324e7f1a1454d830df91c6b78f317)、*奠基*中，我们提到，在Kotlin中，使用结构等式运算符(= = =)将始终调用引擎盖下的`equals`方法，这意味着我们可以轻松安全地比较我们的`Product`数据类的实例:
...
请注意，最后一个常量(`VIOLET`)后面是分号。这是Kotlin代码中实际需要分号的罕见情况。它将常量定义与成员定义分开:
