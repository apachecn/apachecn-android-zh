...
Kotlin 非常重视设计模式。之前，我们已经看到了如何通过对象声明来简化 Singleton 模式的使用，以及如何通过高阶函数和函数类型来简化 Observator 模式的使用。此外，由于 lambda 表达式和函数类型，Kotlin 简化了大多数函数模式的使用。在本章中，我们将看到委托和装饰模式的使用是如何由于类委托而被简化的。我们还将看到一个在编程世界中相当新的特性——属性委托——以及如何使用它来使 Kotlin 属性更加强大。
...
还值得注意的是，Cocoa(苹果公司用于构建软件程序在 iOS 上运行的 UI 框架)经常使用委托而不是继承。这种模式越来越流行，在 Kotlin 得到了高度支持。
...
*   更快的类初始化导致更快的应用启动时间，因为值初始化被延迟到第一次使用时
...
否则，构建`MainPresenter`对象的尝试将在类创建期间发生，此时我们还不能从意图中读取值，并且它不能填充`doctor`属性，应用将崩溃。
...
`notNull`委托提供了与`lateinit`几乎相同的效果(只是错误信息不同)。如果在首先设置值之前尝试使用该属性，它将抛出`IllegalStateException`并终止安卓应用。因此，只有当我们知道在第一次尝试使用之前会设置一个值时，才应该使用它。
...
这是委托属性用法的一个非常简单的例子，也是对属性委托如何工作的一个很好的介绍。委托属性是非常强大的结构，具有多个应用。
...
*   它将定义函数结构，因此我们可以在 AndroidStudio 中生成适当的方法。
...
实例可见库`ActivityStarter`([https://github.com/MarcinMoskala/ActivityStarter](https://github.com/MarcinMoskala/ActivityStarter))Kotlin 配套部分。活动参数是使用注释定义的，但是我们可以使用属性委托来简化 Kotlin 的用法，并允许属性定义为可能的只读，而不是`lateinit`:
...
这样，不可接受的参数定义会在对象初始化期间引发适当的错误，而不是在意外情况下中断应用。
...
关于不同特性及其用法的知识是不够的——还需要了解如何将它们结合起来构建出色的应用。在下一章中，我们将编写一个演示应用，并解释如何将本书中描述的各种 Kotlin 特性结合在一起。