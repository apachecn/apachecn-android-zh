...
实例可见库`ActivityStarter`([https://github.com/MarcinMoskala/ActivityStarter](https://github.com/MarcinMoskala/ActivityStarter))Kotlin配套部分。活动参数是使用注释定义的，但是我们可以使用属性委托来简化 Kotlin 的用法，并允许属性定义为可能的只读，而不是`lateinit`:
