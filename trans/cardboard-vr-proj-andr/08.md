...
为了构建这个项目，我们将使用在[第 5 章](05.html "Chapter 5. RenderBox Engine")、*RenderBox引擎*中创建的`RenderBox`库。您可以使用您的，或者从本书提供的可下载文件或我们的 GitHub 存储库中获取一份副本(使用提交标记的`after-ch7`—[https://GitHub . com/cardbookr/renderboxlib/releases/tag/after-ch7](https://github.com/cardbookvr/renderboxlib/releases/tag/after-ch7))。关于如何导入`RenderBox`库的更详细描述，请参考第 5 章、*RenderBox引擎*的最后一节*在未来项目中使用RenderBox*。要创建新项目，请执行以下步骤:
...
要锁定头部位置，我们只需要在`RenderBox`中禁用头部跟踪。如果您的版本`RenderBox`(内置于[第 5 章](05.html "Chapter 5. RenderBox Engine")、*RenderBox引擎*)还没有这个功能，请将其添加到您单独的`RenderBoxLib` lib 项目中，如下所示:
