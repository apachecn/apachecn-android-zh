...
1.  Android Studio打开后，创建一个新项目。让我们将其命名为`CardboardBox`，并以**空活动**为目标**安卓 4.4 KitKat (API 19)** 。
...
在**项目文件**层次视图中，在Android Studio的左侧，找到`app/res/`资源文件夹，右键点击，进入**新建** | **安卓资源目录**。在**新建资源目录**对话框中，从**资源类型:**中选择 **Raw** ，然后点击**确定**。
...
Gradle 将完成它的构建工作。选择Android Studio窗口底部的**渐变控制台**选项卡，查看渐变构建消息。然后，假设一切顺利，APK 文件将被安装在您连接的手机上(它已连接并打开，对吗？).选择底部的**运行**选项卡，查看上传和启动消息。
...
注意，当你写`Matrix`的时候，Android Studio会想自动导入包。确保你选择的导入是`android.opengl.Matrix`，而不是其他一些矩阵库，比如`android.graphic.Matrix`。
...
在Android Studio，在左侧的安卓项目层次窗格中，找到你的 Java 代码文件夹(比如`com.cardbookvr.cardboardbox`)。右键点击，进入**新建** | **Java 类**。然后，设置**名称:立方体**，点击**确定**。然后，编辑该文件，如下所示(请记住，本书中项目的代码可从出版商的网站和本书的公共 GitHub 存储库中下载):
