...
`sensor`应用程序与前几章的应用程序有几个相似之处。与第 4 章、*中的`fram`应用程序一样，利用 I2C* 存储和检索数据，传感器应用程序使用自己从`AsyncTask`、`HardwareTask`派生的类，从 PacktHAL 对底层传感器接口功能进行 JNI 调用。与硬件的接口由应用程序用户按下的按钮的`onClick()`处理器触发，类似于`gpio`和`fram`应用程序所做的。
