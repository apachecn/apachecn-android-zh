...
由于您的应用程序无法在没有一些严重权限更改的情况下以 root 权限执行，因此您将无法使用内存映射来访问 GPIOs。这实际上限制了您只能对应用程序使用文件输入/输出。
...
PacktHAL 必须被构建到一个库中，并包含在应用程序的项目代码库中，以便应用程序使用。假设您在`$HOME`目录中解压缩了`PacktHAL.tgz`文件，您可以使用以下命令将打包代码复制到应用程序的项目目录(`$PROJECT`)中:
