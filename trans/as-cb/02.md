...
1.  启动安卓工作室，开始新的安卓工作室项目。为您的应用命名`CloudOrder`，并在**公司域名**字段中输入`packtpub.com`或任何其他最适合您或您的公司的名称。然后，点击下一步按钮。
...
15.  要在我们的应用中显示订单，我们必须为其创建布局。展开`layout`文件夹，双击`activity_main.xml`文件打开。默认情况下，会显示布局的预览。点击安卓工作室底部的**文本**标签，将视角改为文本，布局显示为 XML。
