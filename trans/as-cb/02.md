...
本章将教你如何构建一个不需要自己后端，而是使用基于云的解决方案的应用程序。
...
许多应用程序需要后端解决方案，例如，允许用户与服务器或彼此通信，就像在社交应用程序中一样，而今天哪个应用程序不是社交应用程序？你也可以想到一个商业应用，例如，一个物流应用。

当然，我们可以编写自己的应用编程接口，将其托管在某个地方，并编写一些安卓代码与之通信，包括查询、缓存以及我们的应用程序需要支持的所有其他功能。不幸的是，开发所有这些可能是一个非常耗时的过程，并且因为这通常是最有价值的资产，所以必须有另一种方法来做到这一点。

好消息是你不必自己做所有这些事情。互联网上有几个现成的移动后端解决方案，例如 QuickBlox、Firebase、谷歌应用引擎和 Parse，这里仅举几个最著名的例子。

这些解决方案中的每一个都能很好地完成特定的事情；虽然，一种解决方案会比另一种更适合。例如，以 QuickBlox 为例，它提供了最快的设置方式，但价格昂贵。它也不像其他选项那样灵活。最近被谷歌收购的 Firebase 是一个非常棒的解决方案，尤其是如果你需要实时支持的话；例如，对于聊天应用程序。被脸书收购的 Parse 没有实时选项，但更灵活，并提供了一些有趣的第三方集成。
...
解析是我最喜欢的，因为它是目前最灵活的解决方案。它没有数据锁定(所有数据都是可导出的)，但它是可扩展的(如果您选择付费计划而不是免费计划)，它适用于所有相关的移动平台，它甚至允许我们创建云模块(在云中运行的方法，可以定期安排和/或您的应用程序可以接近)。从所有可用的流行服务中，这一个提供了最简单的方法来将后端连接到移动应用程序。

### 注

未来，如果谷歌应用引擎(顺便说一下，它也可以用于 iOS 应用)与安卓工作室的集成得到进一步改善，这种情况可能会改变，尤其是对安卓开发者而言。您可以在**构建**菜单中找到**将模块部署到应用引擎**选项。

# 设置解析

想象一个这样的场景:在一个中心点，订单正在被收集，并将为运输做准备。货物需要交付，客户一旦收到他们订购的货物，就需要在应用程序中登录。每个驱动程序都有一个移动设备和一个应用程序来数字支持这个过程。

这是我们将为其提供接下来三个配方的过程，我们将对其使用 Parse，因为它是我们将要创建的解决方案的最合适的后端。

即将发布的食谱描述了如何设置 Parse，如何将 Parse 中的数据消费到您的安卓应用程序中，以及如何将数据(如签名)从应用程序发送到 Parse。

## 做好准备

要完成这个食谱，你需要安卓工作室的启动和运行以及互联网接入。这就是所有人。

## 怎么做...

让我们先创建一个连接到 Parse 后端的应用程序，这样我们就有了一个基础来构建我们的应用程序。让我们给我们的应用起名`CloudOrder`。进一步的步骤如下:

1.  启动安卓工作室，开始新的安卓工作室项目。为您的应用程序命名`CloudOrder`，并在**公司域名**字段中输入`packtpub.com`或任何其他最适合您或您的公司的名称。然后，点击下一步按钮。
...
3.  接下来，选择**空白活动**，点击**下一步**按钮。在下一页，只需点击**完成**按钮。你的新项目将由安卓工作室建立。
4.  现在，让我们去www.parse.com 创建一个新账户和一个应用。注册[www.Parse.com](http://www.Parse.com)。输入您的姓名、电子邮件地址和选择的密码，然后点击**注册**按钮。
5.  [www.Parse.com](http://www.Parse.com)下一页是**入门**页。在显示您的应用程序名称提示的字段中输入`CloudOrder`或类似的内容。为**公司类型**选择一个最适合您情况的值，并根据选择的值，完成任何其他字段。完成后，点击**开始使用解析**按钮。选择**数据**作为您想要开始使用的产品。选择**移动**作为你的环境。接下来，选择一个平台。选择**安卓**，在下一个视图中，选择**原生(Java)** 选项。
6.  选择**现有项目**选项。我们正在创建一个新项目；然而，为了知道这里发生了什么，我们将自己做以下事情。
7.  现在，下载软件开发工具包。下载时，切换到安卓工作室，将项目视图视角从**安卓**改为**项目**。然后，展开`app`文件夹。请注意，其中一个底层文件夹名为`libs`。
8.  将`Parse-x.x.x.jar`文件(其中`x.x.x`表示版本号)拖放到安卓工作室的`libs`文件夹中。如果出现**非项目文件访问**对话框，只需点击**确定**按钮。当您这样做时，`Parse-x.x.x.jar`将出现在`libs`文件夹下。
...
让我们看看如何通过使用 Parse 后端的订单来扩展我们的`CloudOrder`应用程序，并在以下步骤的帮助下使用列表视图显示它们:
...
11.  运行应用程序并检查**日志文件**(使用快捷方式 *Cmd* + *6* 使其出现)。它显示已找到的对象数量。这将返回您在 www.parse.com 为**货箱**创建的行数。
...
如果没有互联网连接，则`saveEventually`方法会将更新存储在本地，这是移动应用程序的常见场景。一旦互联网连接恢复，这种方法将利用，发送已经排队等待发送到云的数据。这个选项会为你节省很多麻烦和时间。
