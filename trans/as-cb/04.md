...
要完成这个食谱，你需要运行Android Studio。还要确保你安装了最新的软件开发工具包，包括安卓穿戴软件开发工具包。您可以在打开 SDK 管理器时检查是否是这种情况。(导航至**工具**菜单，**安卓 SDK 管理器**)如下图截图所示:
...
1.  开始一个新的Android Studio项目。命名您的应用`WatchApp`，并在**公司域**字段中输入`packtpub.com`。然后，点击**下一步**按钮。
...
*   参考[第一章](01.html "Chapter 1. Welcome to Android Studio")*中的【用名为 Genymotion* 的模拟器测试你的应用】欢迎来到Android Studio
...
要完成这个食谱，你需要启动并运行Android Studio。还要确保你安装了最新的软件开发工具包，包括安卓穿戴软件开发工具包。打开 SDK 管理器可以检查是否是这种情况，当您导航到 **SDK 管理器**菜单项下的**安卓**下的**工具**菜单时，可以访问该管理器。
...
9.  Android Studio将为手机或平板电脑以及可穿戴设备创建必要的模块。
...
17.  你现在会看到Android Studio为你打造的数字手表脸。
...
为了以后为 Play Store 准备手表表面，您需要在完成后截图。你需要提供方形和圆形手表的截图。在`res/drawable`文件夹中，你会找到Android Studio为你创建的默认预览图片。
...
这个食谱需要安装Android Studio和最新的软件开发工具包，包括穿戴软件开发工具包。查看之前的食谱了解更多信息。
...
7.  打开`activity_water_now.xml`文件，使用Android Studio底部的标签将其更改为**文本**模式。
