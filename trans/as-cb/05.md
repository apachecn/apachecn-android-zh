...
要浏览这个食谱，你需要让Android Studio启动并运行，以及一部手机、平板电脑和/或平板设备(物理设备一如既往地被推荐；但是，您可以使用 Genymotion 来创建虚拟的)。
...
1.  开始一个新的Android Studio项目。命名您的应用`YouTubeMediaApp`，并在**公司域**字段中输入`packt.com`。点击**下一步**按钮。
2.  在下面的对话框中，只需勾选**电话和平板**选项，点击**下一步**按钮。
3.  在下一个对话框中，选择**空白活动**，点击**下一步**按钮。
4.  在**自定义活动**对话框中，点击**完成**按钮。
5.  Android Studio将为您创建新项目。在Android Studio左侧的**项目**视图中，在`app`文件夹中找到`build.gradle`并将其打开。
...
21.  Since this app is for demo purposes only, we do not need to look up the requested **SHA1** value. Just click on the **Create** button.
    ...
    ![How to do it...](graphics/B04299_05_01.jpg)
...
这个食谱需要Android Studio启动和运行，并安装最新的软件开发工具包。在本食谱中，我们将为您简要介绍一款电视应用。只需几步，我们将创建一个媒体中心应用。别担心，你不需要安卓电视。我们将创建一个虚拟的。
