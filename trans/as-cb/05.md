...
1.  开始一个新的安卓工作室项目。命名您的应用`YouTubeMediaApp`，并在**公司域**字段中输入`packt.com`。点击**下一步**按钮。
...
这个食谱需要安卓工作室启动和运行，并安装最新的软件开发工具包。在本食谱中，我们将为您简要介绍一款电视应用。只需几步，我们将创建一个媒体中心应用。别担心，你不需要安卓电视。我们将创建一个虚拟的。
