...
有时，许多开发人员预加载所有资产并从缓存中使用它。这种方法使游戏更加流畅和快速。然而，如果发生中断，在该特定游戏状态不需要的缓存中加载资产可能会使游戏崩溃。安卓操作系统完全有权清除非活动或最小化应用占用的内存。当中断发生时，游戏进入最小化状态。如果一个新的应用需要内存，并且没有可用的空闲内存，那么安卓操作系统会杀死不活动的应用，并为新的应用释放内存。
