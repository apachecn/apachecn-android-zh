...
第一步是给我们的项目添加一个新类。为此，我们将创建一个名为`BookListFragment2`的新类，它扩展了`ListFragment`类。在[第 1 章](1.html "Chapter 1. Fragments and UI Modularization")、*片段和 UI 模块化*中，我们手动创建了片段类。对于`BookListFragment2`课程，我们将使用Android Studio。
