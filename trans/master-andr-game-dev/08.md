...
该库需要添加到主模块的`build.gradle`文件中。请注意，Android Studio项目为每个模块包含一个顶级`build.gradle`和一个模块级`build.gradle`。我们将修改`mobile`模块下的一个。
...
`android-basic-samples`下载完成后，在Android Studio打开你的游戏项目。点击**文件** > **导入模块**，导航至下载`android-basic-samples`的目录。在`BasicSamples/libraries`目录中选择`BaseGameUtils`模块，点击**确定**。
