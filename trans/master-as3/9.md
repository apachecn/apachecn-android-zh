...
开发一个移动应用，即使是一个相对简单的应用，也是一个漫长的过程，一旦我们测试了所有的代码，消除了任何障碍，并完善了我们的用户界面，我们就希望能够尽快简单地将我们的产品上架。安卓工作室将所有这些流程整合到工作空间中。
...
每次我们签署一个应用或口味时，都会自动为我们创建签名配置。幸运的是，这些配置可以根据我们的特定目的进行定制。例如，您可能希望在每次构建时自动签署应用。安卓工作室通过项目结构实现了这一点...，可以在主“文件”菜单中找到。
...
利用安卓工作室简单的代码完成和重构功能，编码也不再是一件苦差事。再加上 Kotlin 作为官方开发语言的加入，选择 Android Studio 对许多移动开发人员来说似乎是唯一的选择。使用安卓工作室，甚至编译和测试应用都可以更快、更容易，当然，使用集成开发环境提供的工具，开发新的令人兴奋的外形，如可穿戴设备和物联网，也变得更容易。
