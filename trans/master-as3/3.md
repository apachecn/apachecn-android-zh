...
在前一章中，我们看到了Android Studio如何为快速简单地设计布局提供许多宝贵的工具。然而，我们只关心静态用户界面的设计。当然，这是必不可少的第一步，但是我们的界面可以而且应该是动态的。此外，根据材质设计指南，用户交互应该使用运动和颜色以图形方式进行说明，以直观地展示正在执行的动作，例如点击按钮产生的波纹动画。
...
我们将从Android Studio如何利用基于材质的视觉主题编辑器和设计支持库来促进材质设计的实现开始这一章。
...
我们需要做出的第一个设计决定之一是，我们希望将什么样的配色方案或主题应用到我们的应用中。关于主题的明暗对比，有一两个材质准则。幸运的是，Android Studio的主题编辑器使得生成符合材质的主题变得非常简单。
...
如果您是Android Studio的新手，您会注意到在您键入时出现的代码完成下拉列表，如下图所示:
...
1.  开始一个新的Android Studio项目。我们将创建以下布局:
