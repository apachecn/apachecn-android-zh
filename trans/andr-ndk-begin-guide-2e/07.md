...
6.  然后，创建OpenSL ES 音频播放器。和往常一样，使用 OpenSL ES 对象，先通过引擎实例化，然后实现。两个接口`SL_IID_PLAY`和`SL_IID_SEEK`是必需的:
...
当一个应用程序结束时，不要忘记释放记录器对象作为所有其他 OpenSL 对象。
